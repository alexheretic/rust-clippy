...
fn detect_extreme_expr<'a, 'tcx>(cx: &LateContext<'a, 'tcx>, expr: &'tcx Expr) -> Option<ExtremeExpr<'tcx>> {
    ...
    let size = match ty.sty {
        ty::TyBool => 8,
        ty::TyInt(ity) => layout::Integer::from_attr(cx.tcx, attr::IntType::SignedInt(ity)).size().bits(),
        ty::TyUint(uty) => layout::Integer::from_attr(cx.tcx, attr::IntType::UnsignedInt(uty)).size().bits(),
        _ => return None,
    };
    ...
    let which = match (&ty.sty, cv) {
        ...
        (&ty::TyInt(_), Constant::Int(i)) if i128::min_value() >> (128 - size) == i as i128 => Minimum,

        (&ty::TyBool, Constant::Bool(true)) => Maximum,
        (&ty::TyInt(_), Constant::Int(i)) if i128::max_value() >> (128 - size) == i as i128 => Maximum,
        (&ty::TyUint(_), Constant::Int(i)) if u128::max_value() >> (128 - size) == i => Maximum,
...
fn node_as_const_fullint<'a, 'tcx>(cx: &LateContext<'a, 'tcx>, expr: &'tcx Expr) -> Option<FullInt> {
    ...
    if let Constant::Int(const_int) = val {
        if cx.tables.expr_ty(expr).is_signed() {
            Some(FullInt::S(const_int as i128))
        } else {
            Some(FullInt::U(const_int))
        }
    } else {
        None
    }
